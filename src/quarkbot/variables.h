...
///Abstract implementation of ValueStream
class IValueStream {
public:
    ...
    /**
     * only valid when previous init() or next() returned true, otherwise UB
     * @return pair: value in binary serialized form
     * @note string is valid until next() or init()
     */
...
public:
    ...
    ///Initializes set by service provider
...
class ValueStream<T> {
public:
    ///Initializes empty stream
    ValueStream() = default;

    ///Initializes stream by service provider
    ValueStream(std::unique_ptr<IValueStream> ptr):_ptr(std::move(ptr)) {}
    ...
    public:
        ...
        iterator() = default;
        ///construct iterator
        iterator(const ValueStream *owner, bool valid):_owner(owner),_valid(valid) {fetch();};
        ...
        void fetch() {
            if (_valid) {
                if constexpr(std::is_same_v<T, std::string_view>) {
                    ...
                    auto itr = tmp.begin();
                    auto end = tmp.end();
                    _val = Serializer::from_binary<T>(itr, end);;
                }
            }
        }
    ...
    ///retrieve internal handle pointer
    IValueStream *get_handle() const {return _ptr.get();}

protected:

    template<typename U> friend class ValueStream;

    std::unique_ptr<IValueStream> _ptr;
};
