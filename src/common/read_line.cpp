...
int ReadLine::global_event_hook() {
    auto inst = curInst;
    if (inst && inst->_force_exit) rl_done = 1;
    return 0;
}
...
void ReadLine::initLibsInternal() {
    ...
    rl_event_hook = &global_event_hook;
...
bool ReadLine::read(std::string &line) {
    ...
    if (_force_exit.exchange(false)) return false;
