...
public: //overwrites
    ...
    ///interrupt current or next reading
    /** Function can be called from different thread
     * This causes that read operation exists with current input (as ENTER would hit).
     * Interrupt function is one-shot. Next read() is not interrupted. If
     * function called outside of read(), it remembers state and exits immediately
...
protected:
    ...
    std::atomic<bool> _force_exit = false;
