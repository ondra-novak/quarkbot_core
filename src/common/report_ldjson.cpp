...
std::string ReportLDJSONBase::get_var(std::string_view var_name) const {
    return _storage->get_var(var_name);
}


Fills ReportLDJSONBase::load_fills(std::size_t limit,std::string_view filter) const {
    return _storage->load_fills(limit, filter);
}


bool ReportLDJSONBase::is_duplicate_fill(const Fill &fill) const {
    return _storage->is_duplicate_fill(fill);
}

VarSet<std::string_view> ReportLDJSONBase::get_vars(std::string_view prefix) const {
    return _storage->get_vars(prefix);
}

VarSet<std::string_view> ReportLDJSONBase::get_vars(std::string_view start, std::string_view end) const {
    return _storage->get_vars(start, end);
}

Fills ReportLDJSONBase::load_fills(Timestamp limit, std::string_view filter) const {
    return _storage->load_fills(limit, filter);
}
Positions ReportLDJSONBase::load_positions(std::string_view filter) const {
    return _storage->load_positions(filter);
}
std::vector<SerializedOrder> ReportLDJSONBase::load_open_orders(const Account &account) const {
    return _storage->load_open_orders(account);
}

Trades ReportLDJSONBase::load_closed(Timestamp limit,std::string_view filter) const {
    return _storage->load_closed(limit, filter);
}

void ReportLDJSONBase::put_order(Timestamp tm,const Order &ord) {
...
void ReportLDJSONBase::put_fill(Timestamp tm,const Fill &fill) {
...
void ReportLDJSONBase::begin_transaction() {
    _storage->begin_transaction();
    tx_beg();
}

void ReportLDJSONBase::commit() {
    _storage->commit();
    tx_end();
}
void ReportLDJSONBase::rollback() {
    tx_beg();
    _storage->rollback();
    tx_rollback();
...
void ReportLDJSONBase::put_var(Timestamp tm,std::string_view name, std::string_view value) {
    tx_beg();
    _storage->put_var(tm, name, value);
    _tx.push_back(new_record(tm, "put_var", {name, value}));
    tx_end();
}
void ReportLDJSONBase::erase_var(Timestamp tm,std::string_view name) {
    _storage->erase_var(tm, name);
    _tx.push_back(new_record(tm, "erase_var", name));
    tx_end();
}
json::value ReportLDJSONBase::new_record(Timestamp tm, std::string_view type, json::value payload) {
    return {
...
void ReportLDJSONBase::tx_beg() {
    _txcnt++;
}

void ReportLDJSONBase::tx_end() {
    if (--_txcnt <= 0) {
        flush();
        _tx.clear();
        _txcnt = 0;
...
void ReportLDJSONBase::tx_rollback() {
    for (auto &x: _tx) {
        x.set("rollback", true);
    }
    tx_end();
