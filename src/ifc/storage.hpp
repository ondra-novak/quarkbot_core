...
public:
    ...
    */
     ...
     */
    virtual std::size_t count_orders(std::string_view exchange_id) = 0;
     ...
     */
    virtual std::span<const std::pair<std::string,std::string> > load_orders(std::string_view exchange_id, std::span<std::pair<std::string, std::string > > space) = 0;


    struct TradingState {
        ///contains time of last fill used for calculation
    ...
    */
    virtual std::span<const Fill> load_fills(std::string_view instrument_id, std::span<Fill> space) = 0;
     ...
     */
    virtual TradingState aggregate_fills(std::string_view instrument_id, std::chrono::system_clock::time_point until_time, const TradingState &initial_state) = 0;

    virtual FeeState aggregate_fees(std::string_view instrument_id, std::chrono::system_clock::time_point until_time, const FeeState &initial_state) = 0;
    ...
    virtual std::size_t key_range(std::string_view key) = 0;
    ///Get key-value pairs with specific prefix
    virtual std::span<const std::pair<std::string, std::string> > key_range(std::string_view key, std::span<std::pair<std::string, std::string> > space) = 0;
