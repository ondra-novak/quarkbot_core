...
void test_select_range() {
    ...
    for (auto v: storage.select_range("data", {10,0}, {20,0})) {
        CHECK_EQUAL(static_cast<std::int64_t>(v.key.ordered), p);
        CHECK_EQUAL(v.data, "v" + std::to_string(p));
        ++p;
    }
    CHECK_EQUAL(p, 20);

    // descending over the same half-open interval [0,50)
    p = 49;
    for (auto v: storage.select_range("data", {50,0}, {0,0}, RangeDirection::descending)) {
        CHECK_EQUAL(static_cast<std::int64_t>(v.key.ordered), p);
        CHECK_EQUAL(v.data, "v" + std::to_string(p));
        --p;
    }
...
///range selects the same records whichever way it is traversed. LevelDBStorage must
///agree with the other backend on every case here.
void test_range_direction() {
    TempDB tmp("range_direction");
    auto mgr = open_fresh(tmp.path);
    Storage storage = mgr.get_storage("s");
    auto tx = storage.write();
